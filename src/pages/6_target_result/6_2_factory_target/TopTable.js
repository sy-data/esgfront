        ...
        return [
            {
                ...
                headerName: "사업장명",
                flex: 1,
                valueGetter: (params) => params.value.attributes.name,
            },
    ...
    const handleSaveButton = async() => {
        ...
        props.data.forEach((item) => {
            Object.entries(item).forEach(([key, value]) => {
                if (['factory', 'baseYear', 'id'].includes(key) || value.status === 'saved') return;
                ...
                if (value.status === 'new' && value.value) promises.push(esgFetch(`/api/factory-goals`, 'POST', requestBody));
                if (value.status === 'edit' && value.value) promises.push(esgFetch(`/api/factory-goals/${value.id}`, 'PUT', requestBody));
                if (value.status === 'edit' && value.value === '') promises.push(esgFetch(`/api/factory-goals/${value.id}`, 'DELETE', requestBody));
            })
        });

        const result = await Promise.all(promises);
        const isError = result.some(item => !item.ok);
        if (isError) alert('일부 사업장별 계획 설정 정보를 저장하는 중 오류가 발생하였습니다.')
    ...
    const processRowUpdate = (updatedRowData) => {
        props.setCompanyTargetData(prev => prev.map(item => {
            Object.entries(updatedRowData).forEach(([key, value]) => {
                if (['factory', 'baseYear', 'id'].includes(key) || item[key].value === value) return;
