    ...
    const handleSaveButton = async() => {
        ...
        props.data.forEach((item) => {
            Object.entries(item).forEach(([key, value]) => {
                ...
                if (value.status === 'new' && value.value) promises.push(esgFetch(`/api/factory-plans`, 'POST', requestBody));
                if (value.status === 'edit' && value.value) promises.push(esgFetch(`/api/factory-plans/${value.id}`, 'PUT', requestBody));
                if (value.status === 'edit' && value.value === '') promises.push(esgFetch(`/api/factory-plans/${value.id}`, 'DELETE', requestBody));
            })
        });
