...
const NoRows1Overlay = () => {
  return (
    <div style={{ textAlign: "center", padding: "5px 0px", backgroundColor: "#808080" }}>
      <div style={{ fontSize: "16px" }}>조회 된 하/폐수 시설 정보가 없습니다.</div>
    </div>
  );
};
...
const TopArea = (Props) => {
  ...
  const [sewageWastwaterInformation, setSewageWastwaterInformation] = useRecoilState(sewageWastwaterInfomationAtom);
  ...
  const processRowUpdate = (newRow, oldRow) => {
    ...
    // 로컬 상태 업데이트
    const updatedRows = sewageWastwaterInformation.map((row) => (row.id === oldRow.id ? updatedRow : row));
    setSewageWastwaterInformation(updatedRows);

    // 로컬 스토리지에 저장
    localStorage.setItem("sewageWastwaterInformation", JSON.stringify(updatedRows));
  ...
  const handleSaveActivityData = () => {
    const storedData = localStorage.getItem("sewageWastwaterInformation");
    console.log(storedData);

    // esgFetch("/api/...")
    //   .then((response) => response.json())
    //   .then((response) => {
    //     setSewageWastwaterInformation(response.data);
    // });
  };
  ...
  return (
    <ContentBody>
      ...
      <CustomDataGrid
        data={sewageWastwaterInformation}
        apiRef={apiRef}
        columns={columns}
        slots={{
