...
import { steamInformationAtom } from "../../../States/3_activtiy_data_states/3-4_steem_activity_atom";
...
const NoRows1Overlay = () => {
  return (
    <div style={{ textAlign: "center", padding: "5px 0px", backgroundColor: "#808080" }}>
      <div style={{ fontSize: "16px" }}>조회 된 스팀 시설 정보가 없습니다.</div>
    </div>
  );
};
...
const TopArea = (Props) => {
  ...
  const processRowUpdate = (newRow, oldRow) => {
    ...
    // 로컬 상태 업데이트
    const updatedRows = steamInformation.map((row) => (row.id === oldRow.id ? updatedRow : row));
    setSteamInformation(updatedRows);

    // 로컬 스토리지에 저장
    localStorage.setItem("steamInformation", JSON.stringify(updatedRows));
  ...
  const handleSaveActivityData = () => {
    const storedData = localStorage.getItem("steamInformation");
