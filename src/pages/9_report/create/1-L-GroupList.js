...
const GroupList = props => {
  ...
  return (
    <div style={{flex: 4, padding: "24px", display: "flex", flexDirection: "column", gap: "14px", borderRight: "1px solid #EEEEEE"}}>
      <SubTitle title={"조직명"} />
      <List component="nav" aria-label="secondary mailbox folder">
        <GroupListItem index={1} type={"법인"} name={"(주) 이스코프"} count={5} />
        <GroupListItem index={2} type={"법인"} name={"(주) 하우스코프"} count={3} />
        <GroupListItem index={3} type={"법인"} name={"(주) 에스물류"} count={4} />
      </List>
    </div>
  )
}
